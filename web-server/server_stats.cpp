...
Stats ServeRequests(istream& input) {
    Stats result;
    // смысл всю строку сразу в цикле читать???
...
int main() {
    ...
    string s = "GET /help /protocol PUT /order sad GET / ";
    istringstream st(s);
    Stats res = ServeRequests(st);
    auto value = res.GetMethodStats();
    /*for(const auto& k : value) {
        cout << k.first << " = " << k.second << endl;
    }*/
    auto uri = res.GetUriStats();
    /*for(const auto& k : uri) {
        cout << k.first << " = " << k.second << endl;
    }
    */
}
